...
namespace Wp {
  ...
  class DigitalDownloadTemplateView extends \Blink\TemplateView {
    protected function get() {
      // Check that the download code is valid.
      $recipient = \Wapo\WapoRecipient::get_or_404(array("id"=>$this->request->get->find("id"),"download_code"=>$this->request->get->find("hash")), "Download not found.");
      
      // Get the promotion.
      $promotion = \Wapo\Promotion::get_or_404(array("id"=>$recipient->wapo->promotion), "Download not found.");
      
      // Check that the file has not expired.
      $now = \DateTime::createFromFormat("m/d/Y H:i A", date("m/d/Y H:i A"));
      $expire_date = new \DateTime($recipient->expire_date);
      $diff = $now->diff($expire_date);
      
      // If time difference > 1 hour, exit.
      if($diff->h > 1) {
        \Blink\raise404("Download not found or has expired.");
      }
      
      // Set the number of times they have downloaded.
      $recipient->downloads += 1;
      
      // If time difference > 1 hour, exit.
      if($recipient->downloads > WpConfig::PROMOTION_MAX_DOWNLOADS) {
        \Blink\raise404("Download limit reached.");
      }
